...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				ToastUtil.getInstance(mContext).showToast("很抱歉，程序出现异常，即将退出!");
				Looper.loop();
			}
		}.start();
		// 保存日志文件
	...
	private String saveCatchInfo2File(Throwable ex) {
		...
		try {
			...
			if (Environment.getExternalStorageState().equals(
				...
				fos.close();
				// 发送给开发人员
				sendCrashLog2PM(file_dir + fileName, sb);
			}
			return fileName;
		} catch (Exception e) {
	 ...
	 * @param sb
	 */
	private void sendCrashLog2PM(String fileName, StringBuffer sb) {
